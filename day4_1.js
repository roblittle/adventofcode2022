...
for(let i=0;i<inputArray.length;i++){
    ...
    // if any value range, is entirely inside the other value range, imcremement rangeOverlaps
    // check a, inside of b . and b, inside of a
    if(parseInt(val1LowerLimit) >= parseInt(val2LowerLimit) && 
        parseInt(val1UpperLimit) <= parseInt(val2UpperLimit)){
        rangeOverlaps += 1;
        continue;
    }
    if(parseInt(val2LowerLimit) >= parseInt(val1LowerLimit) && 
    parseInt(val2UpperLimit) <= parseInt(val1UpperLimit)){
        rangeOverlaps += 1;
        continue;
    }
